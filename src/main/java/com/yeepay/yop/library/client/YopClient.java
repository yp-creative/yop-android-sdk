...
import com.yeepay.yop.library.utils.ResponseDecrypt;
...
//    private static HashMap<String,String>map=new HashMap<String, String>();
    ...
    public static String postData(String methodUrl, Map<String,String> params,InputStream[] is){
        Map<String,String>encodeParams=new HashMap<>();
        for(Map.Entry<String,String>entry:params.entrySet()){
            try {
                encodeParams.put(entry.getKey(),URLEncoder.encode(entry.getValue(),"UTF-8"));
        ...
        postMap.put(YopConstants.TIMESTAMP,timestamp);
        postMap.put(YopConstants.LOCALE,"zh_CN");
        postMap.put(YopConstants.FORMAT,"json");
        postMap.put("method",getPostUrl());
        ...
        hashMap.put("X-YOP-Request-ID", UUIDUtils.getUUID());
        Log.e("RequestId",UUIDUtils.getUUID());
...
//         return responseResult;
    ...
    public static String uploadFile(String uploadUrl,Map<String,String> businessMap,InputStream[]is){
...
//        }
        ...
        headerMap.put("X-YOP-Request-ID", UUIDUtils.getUUID());
        try {
          Response response=OkHttpUtils.post()//
                    .addFile("_file", "_file", new File(fileParamMap.get("_file")))//
                    .url(serverUrl+uploadUrl)
                    .params(postMap)//
                    .headers(headerMap)//
                    .build()//
                    .execute();
...
//        }
    ...
    public static void signAndEncrypt(Map<String,String> params){
        ...
        timestamp=map.get(YopConstants.TIMESTAMP);
...
//                builder.append(paramName).append(URLEncoder.encode(signParams.get(paramName),YopConstants.ENCODING));
        ...
        builder.append(YopClient.getSecretKey());
        String result=Digest.digest(builder.toString(),algName);
        Log.e("sign",result);
        return result;
    }
    ...
    public static String decryptParams(String encryptString){
        ...
        try {
            Gson json=new Gson();
            ResponseDecrypt responseDecrypt=json.fromJson(encryptString,ResponseDecrypt.class);
            String result=responseDecrypt.getResult();
            String state=responseDecrypt.getState();
            String ts=responseDecrypt.getTs();
...
//                String encryptResult=(String)jsonObject.get("result");
                if(!(appKey==null)){
                    decryptResult=AESEncrypt.decrypt(result,getSecretKey());
                }else if(!(customerNo==null)){
                    decryptResult=BlowfishEncrypter.decrypt(result,getSecretKey());
                }
